...
for command in cargo cc cp git ldd nproc pkg-config readelf rustc sed sha256sum stat; do
...
if [[ -n "${external_prefix_input}" ]]; then
    ...
    export PKG_CONFIG_PATH="${prefix_dir}/lib/pkgconfig:${prefix_dir}/lib64/pkgconfig${PKG_CONFIG_PATH:+:${PKG_CONFIG_PATH}}"

    for dependency in libavformat libavcodec libavfilter libavutil libswscale libswresample; do
        if [[ ! -f "${prefix_dir}/lib/pkgconfig/${dependency}.pc" \
            && ! -f "${prefix_dir}/lib64/pkgconfig/${dependency}.pc" ]]; then
